...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      ...
      focus:outline-none focus:ring-2 focus:ring-[rgba(250,204,21,0.3)] focus:ring-offset-2 
      focus:ring-offset-[#1c1e26] transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(250,204,21,0.35)] border-[rgba(250,204,21,0.85)]"
      : "border-white/20 hover:border-[rgba(250,204,21,0.45)]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[rgba(250,204,21,0.9)]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[rgba(250,204,21,0.9)] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[rgba(250,204,21,0.85)]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
