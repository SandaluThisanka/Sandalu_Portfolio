...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      ...
      focus:outline-none focus:ring-2 focus:ring-[#f6c500] focus:ring-offset-2 
      focus:ring-offset-[#05060c] transition-all duration-300 peer
    `;

    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(246,197,0,0.35)] border-[#f6c500]"
      : "border-white/20 hover:border-[#f6c500]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#f6c500]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#f6c500] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#f6c500]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
